  ...
  if (!hasDatabaseUrl()) {
    return (
      <section className="bg-public-page px-4 py-16 sm:px-6 lg:px-8">
        <div className="public-card mx-auto max-w-3xl p-6">
          <p className="text-sm font-bold uppercase text-accent">Статус заявки</p>
          <h1 className="mt-3 text-3xl font-bold text-public-primary">База даних не налаштована</h1>
  ...
  return (
    <section className="bg-public-page px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[1fr_340px]">
        <main className="public-card p-6">
          <p className="text-sm font-bold uppercase text-accent">Статус заявки</p>
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h1 className="text-3xl font-bold text-public-primary">{request.requestNumber}</h1>
          ...
          <div className="public-callout mt-6 rounded-lg p-5">
            <h2 className="text-xl font-bold text-public-primary">{statusMeta.label}</h2>
            <p className="mt-2 text-sm leading-6 text-public-muted">{statusMeta.description}</p>
          </div>
          ...
          <div className="mt-6 rounded-md border border-public-border bg-public-elevated p-4">
            <p className="text-xs font-bold uppercase text-public-subtle">Короткий опис</p>
            <p className="mt-2 line-clamp-6 whitespace-pre-wrap text-sm leading-6 text-public-secondary">{request.description}</p>
          </div>

            <p className="mt-6 rounded-md border border-public-border bg-public-section p-4 text-sm leading-6 text-public-muted">
        ...
        <aside className="public-card p-6">
          ...
          <div className="mt-5 grid gap-4">
            {timeline.map((item) => (
              ...
              <p className="text-sm font-bold text-public-primary">{REQUEST_STATUS_LABELS[item.newStatus]}</p>
              <p className="mt-1 text-xs text-public-muted">{formatDate(item.createdAt)}</p>
              </div>
            ))}
          </div>
        </aside>
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-public-border bg-public-section p-4">
      <p className="text-xs font-bold uppercase text-public-subtle">{label}</p>
      <p className="mt-2 text-sm font-semibold text-public-secondary">{value}</p>
    </div>
  );
}
