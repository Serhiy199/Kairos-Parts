    ...
    <section className="grid gap-5 rounded-lg border border-border bg-card p-5 shadow-card sm:p-6">
      ...
      <div className="grid gap-3">
        {types.map((type) => canEdit ? (
          <form key={type.id} action={updateEquipmentType} className="grid gap-3 rounded-md border border-border p-4 lg:grid-cols-[minmax(0,1fr)_minmax(11rem,14rem)_5rem_auto_auto] lg:items-end">
            <input type="hidden" name="id" value={type.id} /><Field label="Назва" name="name" defaultValue={type.name} required /><Field label="Slug" name="slug" defaultValue={type.slug} required /><Field label="Порядок" name="sortOrder" type="number" defaultValue={String(type.sortOrder)} invalid={orderInvalid} errorId="equipment-type-order-error" compact /><label className="flex min-h-11 cursor-pointer items-center gap-2 whitespace-nowrap rounded-md px-2 text-sm font-semibold focus-within:ring-2 focus-within:ring-accent"><input type="checkbox" name="isActive" defaultChecked={type.isActive} className="size-4" /> Активний</label><button className="min-h-11 rounded-md border border-accent px-4 text-sm font-bold">Зберегти</button><p className="text-xs text-muted lg:col-span-5">Виробників: {type._count.manufacturers} · Використань: {usageMap.get(type.name.toLocaleLowerCase('uk-UA')) ?? 0}</p>
          </form>
