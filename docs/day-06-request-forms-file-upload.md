...
- client-side validation для file type і file size перед submit;
...
- `contactName`;
- `companyName` як сумісний alias для назви компанії;
- `phone`;
- `email`;
- `description`;
- `equipmentType`;
...
- створює `Request` зі статусом `NEW` і `source = WEBSITE`;
- якщо користувач авторизований як `CLIENT`, прив'язує заявку до його `ClientProfile`, але source все одно залишається `WEBSITE`, бо заявка створена з публічної форми;
