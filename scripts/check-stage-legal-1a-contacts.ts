...
assert.equal((pageSource.match(/label: 'EMAIL'/g) ?? []).length, 1);
assert.equal((pageSource.match(/siteContacts\.email\.display/g) ?? []).length, 2);
assert.doesNotMatch(pageSource, /companyLegalDetails\.email\.display/);
...
assert.match(pageSource, /Приймаються поштою за зазначеною в блоці контактів юридичною адресою\./);
assert.match(pageSource, /lg:grid/);
